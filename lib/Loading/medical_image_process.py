...
def load_mat_medical_image(path, type=None,
                       viz3d=False, rescale=None, normalization='full_volume_mean',
                       clip_intenisty=True, crop_size=(0, 0, 0), crop=(0, 0, 0), ):

    if type == 'label':
        img_nii = scipy.io.loadmat(path)['new_contour'].astype(float)
    elif type == 'growth':
        growth = torch.from_numpy(scipy.io.loadmat(path)['growth'].astype(float))
        if growth == 0:
            growth = torch.FloatTensor([1.,0])
        else:
            growth = torch.FloatTensor([0,1.])
        return growth
    else:
        img_nii = scipy.io.loadmat(path)['new_img'].astype(float)

    img_np = np.squeeze(img_nii)
...
def load_mat_medical_image_test(path, type=None,
                       viz3d=False, rescale=None, normalization='full_volume_mean',
                       clip_intenisty=True, crop_size=(0, 0, 0), crop=(0, 0, 0), ):
...
def load_affine_matrix(path, type='MRI'):
    """
