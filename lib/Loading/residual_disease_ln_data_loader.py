...
# updated from feeding tube loading data loader

class Residual_Disease_LN(Dataset):
    """
    ...
    def __init__(self, args, mode, 
        ...
        if self.mode == 'train':
            ...
            train_mrn = list(set([x.split('_')[0] for x in self.args.train_ln_list]))
            ...
            train_mrn_str = f"train_ln_mrn_{self.args.random_value_for_fold[self.args.cv_count_csv_save]}.csv"
            ...
            image = self.args.train_ln_list
            labels = [int(x.split('_')[-1].split('-')[-1]) for x in self.args.train_ln_list]

            print('Length of training inputs: ', len(self.args.train_ln_list))
            print('Length of training image labels: ', len(labels))
            assert len(image) == len(labels), 'input and labels should be of same length'

        if self.mode == 'val':
            # Right now will not need. will save csv
            val_mrn = list(set([x.split('_')[0] for x in self.args.val_ln_list]))
            df_val = pd.DataFrame()
            df_val['mrn'] = val_mrn

            # save file as csv
            save_location = r'/data/maia/mdohop/Holman_Pathway/Residual_Disease/Pytorch/Data/RN_pretrain_clinic/'
            val_mrn_str = f"val_ln_mrn_{self.args.random_value_for_fold[self.args.cv_count_csv_save]}.csv"
            df_val.to_csv(os.path.join(save_location, val_mrn_str), index=False)

            # this will be needed for current code (need to take into account split in Train_Transfer_Clinic)
            image = self.args.val_ln_list
            labels = [int(x.split('_')[-1].split('-')[-1]) for x in self.args.val_ln_list]
        ...
        if self.mode == 'test':
            test_mrn = list(set([x.split('_')[0] for x in self.args.test_ln_list]))
            df_test = pd.DataFrame()
            df_test['mrn'] = test_mrn

            # save file as csv
            save_location = r'/data/maia/mdohop/Holman_Pathway/Residual_Disease/Pytorch/Data/RN_pretrain_clinic/'
            test_mrn_str = f"test_ln_mrn_{self.args.random_value_for_fold[self.args.cv_count_csv_save]}.csv"
            df_test.to_csv(os.path.join(save_location, test_mrn_str), index=False)

            # this will be needed for current code (need to take into account split in Train_Transfer_Clinic)
            image = self.args.test_ln_list
            labels = [int(x.split('_')[-1].split('-')[-1]) for x in self.args.test_ln_list]
    ...
    def __getitem__(self, index):
        ...
        assert int(f_img.split('_')[-1].split('-')[-1]) == f_label, 'check that labels match'

        # setting up loading input image; assigning label value
        # self.args.images_to_load = ['pet_ct_[', 'pet_[', 'bed']
        img, lab = np.zeros((self.args.input_H, self.args.input_W, self.args.input_D, len(self.args.images_to_load))), f_label
        ...
        for i, img_load_name in enumerate(self.args.images_to_load):
            ...
            select_image_name = [x for x in os.listdir(mrn_data_dir) if img_load_name in x if 'prert' not in x.lower()]
            ...
            # obtaining bounding box for selected lymph node; will need to come back to implement larger size area
            ln_name = f_img.split('_')[1]
            # for now only using the tight bounding box coordinates, will make 'coord' a variable in the future
            p_id = f_img.split('_')[0]


            temp_img = temp_img[
                       int(self.args.data_key[p_id]['coord'][ln_name]['y'][0]):
                       int(self.args.data_key[p_id]['coord'][ln_name]['y'][1]),
                       int(self.args.data_key[p_id]['coord'][ln_name]['x'][0]):
                       int(self.args.data_key[p_id]['coord'][ln_name]['x'][1]),
                       int(self.args.data_key[p_id]['coord'][ln_name]['z'][0]):
                       int(self.args.data_key[p_id]['coord'][ln_name]['z'][1])]

            if 'pet_[' in img_load_name:
                temp_img = np.clip(temp_img, a_min=0, a_max=self.args.image_min_max[img_load_name.replace('[', 'reshape')][1])
        ...
        if not os.path.exists(os.path.join(r'/data/maia/mdohop/Holman_Pathway/Residual_Disease/Pytorch/Data/saved_images/ln_eval/initial',
                                                   self.args.short_note)):
            os.makedirs(os.path.join(r'/data/maia/mdohop/Holman_Pathway/Residual_Disease/Pytorch/Data/saved_images/ln_eval/initial',
                                                   self.args.short_note))
        else:
            pass

        if random.randint(0, 1000) > 0 and \
                mrn not in os.listdir(os.path.join(r'/data/maia/mdohop/Holman_Pathway/Residual_Disease/Pytorch/Data/saved_images/ln_eval/initial',
                                                   self.args.short_note)):
            multi_reg_image_review(img[..., 0], img[..., 1], img[..., 1],
                                   gaps=3, initial=0, image_rows=4,
                                   mrn=mrn+'_'+self.mode + '_initial', view=False,
                                   fig_storage_dir=os.path.join(r'/data/maia/mdohop/Holman_Pathway/Residual_Disease/Pytorch/Data/saved_images/ln_eval/initial',
                                                   self.args.short_note))

        assert np.shape(img) == (self.args.input_H, self.args.input_W, self.args.input_D, len(self.args.images_to_load)), \
        f" check image shape {np.shape(img)}"


        if self.args.do_normalization.lower() == 'true':

            # TODO: see if need to min max on the particular lymph node image
            # Will use min and max of the lymph node patches for now
            for i, img_load_name in enumerate(self.args.reshape_image_names):

                # Conditioning normalization parameters
                if img_load_name != 'bed_reshape' and img_load_name != 'pet_reshape':
                ...
                if self.mode == 'train':
                    assert min_ <= np.min(img[..., i]), f"{img_load_name} global min {min_} vs local min {np.min(img[..., i])}"
                    assert max_ >= np.max(img[..., i]), f"{img_load_name} global max {max_} vs local max {np.max(img[..., i])}"

                img[..., i] = (img[..., i] - min_)/(max_ - min_)

            if not os.path.exists(os.path.join(
                    r'/data/maia/mdohop/Holman_Pathway/Residual_Disease/Pytorch/Data/saved_images/ln_eval/norm',
                    self.args.short_note)):
            ...
            if random.randint(0, 1000) > 0 and \
                mrn not in os.listdir(os.path.join(r'/data/maia/mdohop/Holman_Pathway/Residual_Disease/Pytorch/Data/saved_images/ln_eval/norm',
                                                   self.args.short_note)):
                multi_reg_image_review(img[..., 0], img[..., 1], img[..., 1],
                                       gaps=3, initial=0, image_rows=4,
                                       mrn=mrn + '_' + self.mode + '_norm', view=False,
                                       fig_storage_dir=os.path.join(r'/data/maia/mdohop/Holman_Pathway/Residual_Disease/Pytorch/Data/saved_images/ln_eval/norm',
                                                   self.args.short_note))
        else:
            pass
        ...
        if not self.clinic_image_eval:
            if self.mode == 'train' and self.augmentation.lower() == 'true':
                ...
                if not os.path.exists(os.path.join(
                        r'/data/maia/mdohop/Holman_Pathway/Residual_Disease/Pytorch/Data/saved_images/ln_eval/aug',
                        self.args.short_note)):
                    os.makedirs(os.path.join(
                        r'/data/maia/mdohop/Holman_Pathway/Residual_Disease/Pytorch/Data/saved_images/ln_eval/aug',
                        self.args.short_note))
                ...
                if random.randint(0, 1000) > 0 and \
                mrn not in os.listdir(os.path.join(r'/data/maia/mdohop/Holman_Pathway/Residual_Disease/Pytorch/Data/saved_images/ln_eval/aug',
                                                   self.args.short_note)):
                    multi_reg_image_review(img[..., 0], img[..., 1], img[..., 1],
                                           gaps=3, initial=0, image_rows=4,
                                           mrn=mrn + '_' + self.mode + '_aug', view=False,
                                           fig_storage_dir=os.path.join(r'/data/maia/mdohop/Holman_Pathway/Residual_Disease/Pytorch/Data/saved_images/ln_eval/aug',
                                                   self.args.short_note))
