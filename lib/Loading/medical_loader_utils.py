...
def fix_seg_map(segmentation_map, dataset="iseg2017"):
    ...
    elif dataset == "utsw_an_0617":
        Tumor = 1
        segmentation_map[segmentation_map == 1] = Tumor
...
def create_mat_sub_volumes(*ls, dataset_name, mode, samples, full_vol_dim, crop_size, sub_vol_path, normalization='max_min',
                       th_percent=0.1):
    """

    :param ls: list of modality paths, where the -2 path is the segmentation map, -1 path is growth or not
    :param dataset_name: which dataset is used
    :param mode: train/val
    :param samples: train/val samples to generate
    :param full_vol_dim: full image size
    ...
    for i in range(samples):
        ...
        label_path = sample_paths[-2]
        crop = find_random_crop_dim(full_vol_dim, crop_size)
        full_segmentation_map = img_loader.load_mat_medical_image(label_path, viz3d=True, type='label',
                                                                crop_size=crop_size,
                                                                crop=crop)
        ...
        # print(full_segmentation_map.shape)
        # if find_non_zero_labels_mask(full_segmentation_map, th_percent, crop_size, crop):
        segmentation_map = img_loader.load_mat_medical_image(label_path, type='label', crop_size=crop_size,
                                                            crop=crop)
        segmentation_map = fix_seg_map(segmentation_map, dataset_name)
        for j in range(modalities - 2):
            img_tensor = img_loader.load_mat_medical_image(sample_paths[j], type="T1", normalization=normalization,
                                                        crop_size=crop_size, crop=crop)
            tensor_images.append(img_tensor)

        growth = img_loader.load_mat_medical_image(sample_paths[-1], type='growth')
        ...
        f_growth = filename + 'growth.npy' 
        np.save(f_growth, growth)
        list_saved_paths.append(f_growth)
...
def create_mat_sub_volumes_test(*ls, dataset_name, mode, samples, full_vol_dim, crop_size, sub_vol_path, normalization='max_min',
                       th_percent=0.1):
    """
    ...
    for i in range(total):
        ...
        crop = find_random_crop_dim(full_vol_dim, crop_size)
        for j in range(modalities):
            img_tensor = img_loader.load_mat_medical_image_test(sample_paths[j], type="T1", normalization=normalization,
                                                        crop_size=crop_size, crop=crop)
            tensor_images.append(img_tensor)
        ...
        f_name = filename + 'name.npy'

        np.save(f_name, str(filename))
        list_saved_paths.append(f_name)
        list.append(tuple(list_saved_paths))
...
def find_non_zero_labels_mask(segmentation_map, th_percent, crop_size, crop):
    ...
    if label_percentage >= th_percent:
        # print("non_zero_labels_mask" + str(crop_voxel_labels) + " in " + str(total_voxel_labels))
        return True
    else:
        print(segmentation_map.shape)
        print(cropped_segm_map.shape)
        print(crop_size)
        print(crop)
        print("not non_zero_labels_mask " + str(crop_voxel_labels) + " in " + str(total_voxel_labels))
        return False
